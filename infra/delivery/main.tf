resource "azurerm_container_app" "delivery-api" {
  name                         = "delivery"
  ...
  template {
    ...
    container {
      name   = "application"
      image  = "plantpowerjames/plant-based-pizza-delivery-api:${var.app_version}"
      cpu    = 0.25
      memory = "0.5Gi"
      env {
...
resource "azurerm_container_app" "delivery-worker" {
  name                         = "delivery-worker"
  container_app_environment_id = data.azurerm_container_app_environment.env.id
  resource_group_name          = data.azurerm_resource_group.plant_based_pizza_rg.name
  revision_mode                = "Single"
  dapr {
    app_id = "delivery-worker"
    app_port = 8080
    app_protocol = "http"
  }
  identity {
  ...
  template {
    ...
    container {
      name   = "application"
      image  = "plantpowerjames/plant-based-pizza-delivery-worker:${var.app_version}"
      cpu    = 0.25
      memory = "0.5Gi"
      env {
        name = "DatabaseConnection"
