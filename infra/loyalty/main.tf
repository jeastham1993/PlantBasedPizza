resource "azurerm_container_app" "loyalty-api" {
  name                         = "loyalty"
  ...
  template {
    ...
    container {
      name   = "application"
      image  = "plantpowerjames/plant-based-pizza-loyalty-api:${var.app_version}"
      cpu    = 0.25
      memory = "0.5Gi"
      env {
...
resource "azurerm_container_app" "loyalty-internal-api" {
  name                         = "loyalty-internal"
  container_app_environment_id = data.azurerm_container_app_environment.env.id
  resource_group_name          = data.azurerm_resource_group.plant_based_pizza_rg.name
  revision_mode                = "Single"
  dapr {
    app_id = "loyaltyinternal"
    app_port = 8080
    app_protocol = "grpc"
  }
  identity {
  ...
  template {
    ...
    container {
      name   = "application"
      image  = "plantpowerjames/plant-based-pizza-loyalty-internal-api:${var.app_version}"
      cpu    = 0.25
      memory = "0.5Gi"
      env {
        name = "DatabaseConnection"
...
resource "azurerm_container_app" "loyalty-worker" {
  name                         = "loyaltyworker"
  container_app_environment_id = data.azurerm_container_app_environment.env.id
  resource_group_name          = data.azurerm_resource_group.plant_based_pizza_rg.name
  revision_mode                = "Single"
  dapr {
    app_id = "loyaltyworker"
    app_port = 8080
    app_protocol = "http"
  }
  identity {
  ...
  template {
    ...
    container {
      name   = "application"
      image  = "plantpowerjames/plant-based-pizza-loyalty-worker:${var.app_version}"
      cpu    = 0.25
      memory = "0.5Gi"
      env {
        name = "DatabaseConnection"
