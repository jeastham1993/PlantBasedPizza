...
public class OrderPrepCompleteEventWorker : BackgroundService
{
    ...
    public OrderPrepCompleteEventWorker(RabbitMqEventSubscriber eventSubscriber, ActivitySource source,
        OrderPrepCompleteEventHandler eventHandler)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        var subscription = _eventSubscriber.CreateEventConsumer(queueName, "kitchen.orderPrepComplete.v1");
        ...
        subscription.Consumer.Received += async (model, ea) =>
        {
            ...
            {
                var evtDataResponse =
                    await _eventSubscriber.ParseEventFrom<OrderPrepCompleteEventV1>(ea.Body.ToArray());
